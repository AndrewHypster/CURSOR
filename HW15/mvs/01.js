...
// Шаблонний рядок
const HW1 = (a = 15.678, b = 123.965, q = 90.2345) => `${max (a, b, q)}<br>${min (a, b, q)}<br>Сума: ${sum (a, b, q)}<br>${floorSum (a, b, q)}<br>${roundHundredths (a, b, q)}<br>${isEven (a, b, q)}<br>${rest (a, b, q)}<br>${average (a, b, q)}<br>Знижка: ${discount()}%<br>Ціна зі знижкою: ${discountedPrice (a, b, q)}<br>${profit (a, b, q)}`;
export default HW1;