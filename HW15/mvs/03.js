...
document.writeln('<h3>Конвертер валют (1$ = 40₴)</h3><p><b>3500₴ в $</b></p>');
convertСurrency('3500₴');
document.writeln('<p><b>3500$ в ₴</b></p>')
convertСurrency('3500$');
document.writeln('<h3>Створення рандомного пароля</h3>');
getRandomPassword(4);
...
function convertСurrency(currency) {
    if(currency.indexOf('₴') >= 0) {
        if (currency == '₴') {
            document.writeln(1/40 + ' $');
            return;
        }
        document.writeln(currency.slice(0, -1)/40 + ' $');
        return;
    }
    else if (currency.indexOf('$') >= 0){
        if (currency == '$') {
            document.writeln(1*40 + ' ₴');
            return;
        }
        ...
        document.writeln('Ви забули ввести валюту ₴ або $');
        return;
    }
}

function getRandomPassword(lenghtPass = 8) {
    const password = [];
    for(let i=0; i<lenghtPass; i++){
        password[i] = Math.round(Math.random());
    }
    document.writeln(password.join(' '));
}
